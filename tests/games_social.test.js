 ...
 *  4) 🚫 v7.20 — بلا ذكاء → صمت تام: لا تعليقات وهمية أبداً (بلاغ المالك:
 *     «انا أردت الذكاء الاصطناعي نفسه لكنك عملت تعليقات وهمية؟ احذفهم!»)
 *     + الفشل نفسه يبقى مرئياً في سجل الأحداث
...
async function run() {
    ...
    // ═══ 4) 🚫 v7.20: بلا ذكاء → صمت تام — لا تعليقات وهمية أبداً ═══
    {
        sessions.__reset();
        sessions.startSession(AGENT_A, GUILD, { gameName: 'روليت' });
        ...
        assert.strictEqual(client.__sent.length, 0, 'فشل الذكاء → صمت تام — صفر تعليقات وهمية (بلاغ المالك: احذفهم!)');
        // والفشل نفسه مرئي للمالك في سجل الأحداث — لا صمت تشخيصي
        const events = await store.getRecentEvents(AGENT_A);
        const failNote = (events || []).find(e => String(e.text || '').includes('استشارة الذكاء فشلت'));
        assert.ok(failNote, 'فشل الذكاء مُبلّغ عنه مرئياً (لا صمت تشخيصي)');
        assert.ok(failNote.text.includes('provider down') || failNote.text.includes('provider'), 'سبب الفشل الحقيقي في التبليغ');
    }
